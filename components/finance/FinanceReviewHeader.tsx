...
interface FinanceReviewHeaderProps {
  ...
  isSaving: boolean;
  isLoading?: boolean;
  userRole?: string; // Add role to conditionally show/hide approve buttons
  onBack: () => void;
  onEdit: () => void;
  onCancelEdit: () => void;
  onSaveOverview: () => void;
...
export function FinanceReviewHeader({
  ...
  // Check if user is sales admin (hide approve buttons)
  const isSalesAdmin = userRole === "SALES_ADMIN" || userRole === "sales_admin";
  return (
        ...
        <div className="flex gap-2 justify-end flex-wrap shrink-0">
          {isEditingOverview ? (
            ...
            <>
              ...
              {/* Only show approve buttons for finance role, not sales admin */}
              {!isSalesAdmin && (
                <>
                  {!isDealApproved ? (
                    <Button
