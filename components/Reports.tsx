...
export function Reports() {
  ...
  const filteredDeals = useMemo(() => {
    ...
    if (selectedAgent !== "all") {
      deals = deals.filter((deal) => deal.agentName === selectedAgent);
    }
  ...
  const summaryMetrics = useMemo(() => {
    const totalRevenue = filteredDeals.reduce(
      (sum, deal) => sum + (deal.receivedAmount || 0),
      0
    );
    const dealsCount = filteredDeals.length;
    const commissionReceived = filteredDeals.reduce(
      (sum, deal) => sum + (deal.receivedAmount || 0),
      0
    );
    const commissionPaidToAgents = filteredDeals.reduce((sum, deal) => {
      const agentCommission = deal.agentCommission || 0;
      const paidPercentage = deal.paidPercentage || 0;
      return sum + agentCommission * (paidPercentage / 100);
    }, 0);
  ...
  const monthlyData = useMemo(() => {
    const monthlyMap = new Map<
      string,
      { revenue: number; deals: number; units: number }
    >();
    ...
    filteredDeals.forEach((deal) => {
      if (deal.dealCloseDate) {
        ...
        monthlyMap.set(monthKey, {
          revenue: existing.revenue + (deal.receivedAmount || 0),
  ...
  const developerData = useMemo(() => {
    const devMap = new Map<
      string,
      { deals: number; commission: number; value: number }
    >();

    filteredDeals.forEach((deal) => {
      const existing = devMap.get(deal.developer) || {
        deals: 0,
        commission: 0,
        value: 0,
      };
      devMap.set(deal.developer, {
        deals: existing.deals + 1,
        commission: existing.commission + (deal.receivedAmount || 0),
        value: existing.value + (deal.sellingPrice || 0),
      });
    });
  ...
  const agentData = useMemo(() => {
    const agentMap = new Map<
      string,
      { deals: number; commission: number; paid: number; unpaid: number }
    >();

    filteredDeals.forEach((deal) => {
      const existing = agentMap.get(deal.agentName) || {
        deals: 0,
        commission: 0,
        paid: 0,
        unpaid: 0,
      ...
      const agentCommission = deal.agentCommission || 0;
      const paidPercentage = deal.paidPercentage || 0;
      const paid = agentCommission * (paidPercentage / 100);
      const received = deal.receivedAmount || 0;
      const unpaid = received - paid;

      agentMap.set(deal.agentName, {
        deals: existing.deals + 1,
        commission: existing.commission + received,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-0 shadow-md hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-green-50 to-white">
          ...
          <CardContent>
            <div className="text-2xl text-gray-900">
              AED {(summaryMetrics.totalRevenue / 1000).toFixed(0)}K
            </div>
            <p className="text-sm text-gray-600 mt-1">
              {filteredDeals.length > 0 ? "Filtered results" : "No data"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-md hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-emerald-50 to-white">
          ...
          <CardContent>
            <div className="text-2xl text-gray-900">
              AED {(summaryMetrics.commissionReceived / 1000).toFixed(0)}K
            </div>
        ...
        <Card className="border-0 shadow-md hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-orange-50 to-white">
          ...
          <CardContent>
            <div className="text-2xl text-gray-900">
              AED {(summaryMetrics.outstanding / 1000).toFixed(0)}K
            </div>
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-100">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-2 max-w-[100px]">
                            <div
                              ...
                              style={{
                                width: `${
                                  agent.commission > 0
                                    ? (agent.paid / agent.commission) * 100
                                    : 0
                                }%`,
                              }}
                            ></div>
                          ...
                          <span>
                            {agent.commission > 0
                              ? ((agent.paid / agent.commission) * 100).toFixed(
                                  1
                                )
                              : 0}
                            %
                ...
                <tbody>
                  {developerData.map((dev) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-100">
                        AED {dev.value.toLocaleString()}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-100 font-medium">
                        AED {dev.commission.toLocaleString()}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-100">
